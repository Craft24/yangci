...
<script>
	...
	function build_grid_tr(){cjTable_tr( 'iqc_report_list?in_id='+$(clickedId).find('td').eq(0).text()+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 'house_id,supplier_ids',0);                        ///表格高度,需要隐藏的td
                            }
    function build_grid_tr_add(in_id){cjTable_tr( 'iqc_report_list?in_id='+in_id+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 'house_id,supplier_ids',1);                        ///表格高度,需要隐藏的td
                            }
    $(function(){
	   $("#head").makemenu2(base_url);////顶部菜单
	   
	   function build_grid(){cjTable( '#grid','iqc_report_list?dates='+Math.floor(Math.random()*9999+1),////url of data source
							 '原料id,品名,全名,规格,规格2,数量,抽检数量,不合格数,不合格率,单位',////表格标题
							 '370px','house_id,customer_ids'   );   ///表格高度,需要隐藏的td
                            }
	   function build_search_grid(search_string){cjTable( '#grid','iqc_report_list?'+string+'&dates='+Math.floor(Math.random()*9999+1),////url of data source
							  '原料id,品名,全名,规格,规格2,数量,抽检数量,不合格数,不合格率,单位',
							 '370px','house_id,customer_ids'   );                                    ///表格高度,需要隐藏的td
                            }
	   build_grid();
