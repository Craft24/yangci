...
<script>
	...
	function build_grid_tr(){cjTable_tr( 'material_in_report_list?in_id='+$(clickedId).find('td').eq(0).text()+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 'house_id,supplier_ids',0);                        ///表格高度,需要隐藏的td
                            }
    function build_grid_tr_add(in_id){cjTable_tr( 'material_in_report_list?in_id='+in_id+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 'house_id,supplier_ids',1);                        ///表格高度,需要隐藏的td
                            }
    $(function(){
	   $("#head").makemenu2(base_url);////顶部菜单
	   
	   function build_grid(){cjTable( '#grid','material_in_report_list?dates='+Math.floor(Math.random()*9999+1),////url of data source
							 '材料id,材料名,材料全名,规格,规格2,数量,单位,金额',////表格标题
							 '370px','house_id,supplier_ids'   );   ///表格高度,需要隐藏的td
                            }
	   function build_search_grid(search_string){cjTable( '#grid','material_in_report_list?'+string+'&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 '材料id,材料名,材料全名,规格,规格2,数量,单位,金额',////表格标题
							 '370px','house_id,supplier_ids'   );                                    ///表格高度,需要隐藏的td
                            }
	   build_grid();
	  ...
	  $("#search").click(function(){
		...
		$("#search_start_date").focus(function(){///自动填写起始日期
         ...
         $("#search_purchase_id").focus(function(){
	                       if ($(this).val()=='采购单号') $(this).val('');
						    });
