...
<script>
	...
	function build_grid_tr(){cjTable_tr( 'product_out_report_list?in_id='+$(clickedId).find('td').eq(0).text()+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 'house_id,supplier_ids',0);                        ///表格高度,需要隐藏的td
                            }
    function build_grid_tr_add(in_id){cjTable_tr( 'product_out_report_list?in_id='+in_id+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 'house_id,supplier_ids',1);                        ///表格高度,需要隐藏的td
                            }
    $(function(){
	   $("#head").makemenu2(base_url);////顶部菜单
	   
	   function build_grid(){cjTable( '#grid','product_out_report_list?dates='+Math.floor(Math.random()*9999+1),////url of data source
							 '产品id,品名,产品全名,规格,规格2,数量,单位,金额',////表格标题
							 '370px','house_id,customer_ids'   );   ///表格高度,需要隐藏的td
                            }
	   function build_search_grid(search_string){cjTable( '#grid','product_out_report_list?'+string+'&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 '产品id,品名,产品全名,规格,规格2,数量,单位,金额',////表格标题
							 '370px','house_id,customer_ids'   );                                    ///表格高度,需要隐藏的td
                            }
	   build_grid();
