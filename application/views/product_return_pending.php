...
<script>
	...
	function build_grid_tr(in_id){
	                         ...
	                         cjTable_tr( 'product_return_pending_list?in_id='+$(clickedId).find('td').eq(0).find('div').eq(0).text()+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 'warehouse,supplier_id,supplier_ids',0);  
							                      
                            }
	function build_grid_tr_add(in_id){cjTable_tr( 'product_return_pending_list?in_id=new'+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 'warehouse,supplier_id,supplier_ids',1);                        ///表格高度,需要隐藏的td
                            }
 
    $(function(){
	   $("#head").makemenu2(base_url);////顶部菜单
	   bound_search_controls('成品');	
	   function build_grid(){cjTable( '#grid','product_return_pending_list?s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 '序号,暂入日期,物料id,品名,全名,规格,规格2,数量,类型,订单号,客户订单号,状态,抽样数,不良数,不良率,结果,检验员,委外供应商,种类,入库,批号,仓库,退货,审核',////表格标题
							 '370px','currency_id,warehouse,supplier_ids'   );                                    ///表格高度,需要隐藏的td
                            }
	   function build_search_grid(search_string){cjTable( '#grid','product_return_pending_list?string='+string+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 '序号,暂入日期,物料id,品名,全名,规格,规格2,数量,类型,订单号,客户订单号,状态,抽样数,不良数,不良率,结果,检验员,委外供应商,种类,入库,批号,仓库,退货,审核',////表格标题
							 '370px','currency_id,warehouse,supplier_ids'   );                                    ///表格高度,需要隐藏的td
                            }
	   build_grid();
      ...
      ////////////新加 handling
		  ...
		  $("#correct").click(function(){////更正prop-up
									 ...
									 var col=$("#table_title").find('tr').eq(0).find('th').filter(":contains('委外供应商')").index();///得到审核内容所在的列数
									  ...
									  $("#update_warehouse_id").val($(clickedId).find('td').eq(col+2).text());
									  $("#update_warehouse").val($(clickedId).find('td').eq(col+1).text());
									   var col=$("#table_title").find('tr').eq(0).find('th').filter(":contains('批号')").index();
									  $("#update_batch").val($(clickedId).find('td').eq(col+1).text());
	  ...
	  $("#form_button_add").click(
	                      function (){
						             ...
						             if( $("#add_order_id").val().length<1) {note('请选择订单号');return false;} 
									 ...
									 if (erro==''){
													  var add_url='product_return_pending_add';///新增操作指向的页面
													  adds_2(add_url);
									                }else{ note(erro);}
	                                });
	  $("#form_button_update").click(
	                      function (){
						             ...
						             if($("#update_order_id").val().length<1) {note('请选择订单号');return false;} 
									 var erro=verify("#form_update",'qty,数量,digital||material_type,品质,required||material_name,品名,required||warehouse_id,仓库,required||warehouse_name,仓库名称,required');
									  if (erro==''){
									  var url='product_return_pending_update?'+data;///操作指向的页面
	                                  updates(url);
									 }else{ note(erro); }
	                                }
	                           );
	  ...
	  $("#approve").click(
	                      function (){
									 ...
									 var col=$("#table_title").find('tr').eq(0).find('th').filter(":contains('审核')").index();///得到审核内容所在的列数
									 if ($(clickedId).find('td').eq(col+3).find('div').eq(0).text()!='') erro='不能再审核';
									 var type='';
...
<div id=main>
  <h2>客户退货</h2><p></p>
    <div id=main_left>
		...
		<button id=out_by_production class=out>新退货</button><button id=correct >更改</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <button id=qc >品检</button><button id=qc_update >更改</button> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" id="approve"  value="入库审核"/>
...
　    <!--查询套件结束 -><--> 	  
 	  <!--用于添加的pop up-->
		<div id="add_hidden" class="pop_up">
		              <div class="div_title">成品退货暂入<div class=title_close>关闭</div></div>
			      <div class="table_margin">
						...
						<tr height=35>
						<td>订单号</td><td><input  type=text id=add_order_id name=order_id class=order_id />
						</td></tr>
						<tr height=35>
						<td>客户订单号</td><td><input  type=text id=add_customer_order_id name=customer_order_id class=customer_order_id readonly />
						</td></tr>
						<tr height=35>
 	  ...
 	  <!--用于添加的pop up-->
		<div id="correct_hidden" class="pop_up">
			      ...
			      <div class="table_margin">
						...
						<tr height=35>
						<td>订单号</td><td><input type=text id=update_order_id name=order_id class=order_id  />
						                  
						</td></tr>
						<tr height=35>
						<td>客户订单号</td><td><input type=text id=update_customer_order_id name=customer_order_id class=customer_order_id readonly />
						...
						<tr height=35>
						<td>类型</td><td><input type=hidden id=update_final class=final name=final value='F' /> <input type=hidden id=update_material_type3 name=nouse   /></td></tr>
						<tr height=35>
      ...
      <!--用于品检更改的pop up--结束 -->
		...
		<div id="transfer_hidden" class="pop_up">
		         <div class="div_title">成品拨转<div class=title_close>关闭</div></div>
			      <div class="table_margin">
