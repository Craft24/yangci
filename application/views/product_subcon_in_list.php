...
<script>
	...
	var search_string;var page_string;var string;
	 function build_grid_tr(){cjTable_tr( 'product_subcon_in_list_show?in_id='+$(clickedId).find('td').eq(0).find('div').eq(0).text()+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							
							 '',0  );                                    ///表格高度,需要隐藏的td
                            }	
    function build_grid_tr_add(in_id){cjTable_tr( 'product_subcon_in_list_show?in_id=new'+'&s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 '',1);                        ///表格高度,需要隐藏的td
                            }
    ...
    $(function(){
	   ...
	   function build_grid(){cjTable( '#grid','product_subcon_in_list_show?s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 '序号,入库日期,物料id,品名,全名,规格,规格2,状态,数量,单位,批号,生产单号,入库,审核,审核日期,成品, ,供应商名,仓库,',////表格标题题
							 '400px','currency_id,warehouse,supplier_ids');                                    ///表格高度,需要隐藏的td
                            }
	   function build_search_grid(search_string){cjTable( '#grid','product_subcon_in_list_show?s=0'+string,////url of data source
							 '序号,入库日期,物料id,品名,全名,规格,规格2,状态,数量,单位,批号,生产单号,入库,审核,审核日期,成品, ,供应商名,仓库,',////表格标题题
							 '400px','currency_id,warehouse,supplier_ids');                                    ///表格高度,需要隐藏的td
                            }
	 ...
	 ///////////反审核						   
	  $("#revocation").click(
	   ...
	   $("#correct_material_name").keyup(function(){
       	  		  ...
       	  		  $("#stat").click(function(){///汇总统计
						   ...
						   $.getJSON('product_subcon_in_list_show?s=1&dates='+Math.floor(Math.random()*9999+1)+'&stat=1&'+string,
									 function(result)
									                { var sum="";
	  ...
	  $("#search").click(function(){
						   ...
						   search_string=search_string+'&warehouse='+$("#search_warehouse_id").val();
						   search_string=search_string+'&supplier_id='+$("#search_supplier_id").val();
						   search_string=search_string+'&way='+$("#search_way").val();
						   string=search_string;//alert(string);
						   build_search_grid();
						               }else{
									   note(erro);
		...
		$("#search_material_name").keyup(function(){
		 ...
		 $("#sub_relation").click(function(){
	                       location.href = "subcon_out_check";
						    });						
	   })  
</script>
<body>
...
<div id=main>
  <h2>半成品委外加工记录</h2><p></p>
	...
	　<!--查询套件 -><-->
		...
		<select id=search_way name=way><option value=''>收入和发出</option><option value='in'>收入</option><option value='out'>发出</option></select>
