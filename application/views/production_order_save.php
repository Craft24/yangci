    ...
    var base_url='<?php echo base_url();?>';
     function material_outs(material_id) {
	  ...
	  function show_job_add(production_id,product_name,target_material_id)
	   ...
	   function build_grid_tr(){cjTable_tr('production_list?s=0&dates='+Math.floor(Math.random()*9999+1),////url of data source
							 'supplier_id,currency_id'   );                                    ///表格高度,需要隐藏的td
                            }						   
    $(function(){
	   ///////////////////////////validate update///
	  ...
	  $("#form_button_update").click(
	                      function (){
									 ...
									 if (erro==''){
									 var update_url='welcome/production_update';///更新操作指向的页面
	  ...
	  $("#form_button_add").click(
	                      function (){
									 ...
									 if (erro==''){
									 var add_url='welcome/production_add';///新增操作指向的页面
	                                 adds(add_url);
									 build_grid();
	                                }else{  note(erro);}
									}
	  ...
	  $("#form_button_job_add").click(
	                      function (){
									 ...
									 if (erro==''){
									 var add_url='welcome/job_add';///新增操作指向的页面
	  ...
	  $("#button_progress").click(
	                      function(){
									 ...
									 var product_name=($(clickedId).find('td').eq(1).find('div').eq(0).html());
									  $("#process_info").parent().empty();$("#consumption_info").parent().remove();
									  $(clickedId).after("<tr><td style='padding-left:100px;' id=process_info colspan=11>工序:</td></tr>");
									 $.getJSON(
									          'the_process',function(result)
											              {
														   ...
														   function bom(id){
																		   ...
																		   for(var i=0;i<results.length;i++){ 
																											if (results[i].product_id==id) 
																											{
																											
																											td+='<div class=f><div class=underline>['+results[i].child_material_id+']x'+results[i].qty+"<img src=../../img/job.png onclick=show_job_add("+production+',"*",'+results[i].child_material_id+") border=0 title='派工'>_</div>";
																											td+=bom(results[i].child_material_id);
																											td+='</div>';
																											list+=','+results[i].child_material_id;
																											}
																											} 
																			td+="</div>";
																			return td;
      ...
      $("#button_search").click(
	    ...
	    $("#button_approve").click(
	                      function (){
									 ...
									 if(erro=='')
									 {
									  	 var url='welcome/production_approve?production_id='+$(clickedId).find('td').eq(0).text();
										 updates(url); 
							  		  }else{
									       note(erro);
									       }					 
	    ...
	    $("#button_revocation").click(
	                      function (){
									 ...
									 if(erro=='')
									 {
									  	 var url='welcome/production_revocation?production_id='+$(clickedId).find('td').eq(0).text();
										 updates(url); 
							  		  }else{
									       note(erro);
									       }					 
    ...
    <div id=main_left>
	    ...
	    <div id=grid>
		   ...
		   <input type=button  id=button_add value=" 新增 " />
		    <input type=button  id=button_correct value=" 更正 " />
...
　    <!--查询套件结束 -><-->
		...
		<div id="update_hidden" class="pop_up">
			      ...
			      <div class="table_margin">
						...
						产品</td><td><input type=text id=update_material_name name=material_name readonly  /></td></tr><TR><td>
						...
						完成期限</td><td><input type=text id=delivery_date name=delivery_date  /></td></tr><tr height=35><td>
 	  ...
 	  <!--用于添加的pop up-->
		<div id="add_hidden" class="pop_up">
			      ...
			      <div class="table_margin">
						...
						产品</td><td><input type=text id=add_material_name name=material_name   readonly /><input type="button" class="get_material" value=...></td></tr><tr height=35><td>
	...
	<div id=show class=pop_up style="padding:0px 4px 4px 4px;" >
        <div class="div_title" >领料记录<div class=title_close>关闭</div></div>
