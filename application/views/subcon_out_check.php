...
<script>
    ...
    $(function(){
	   ...
	   function build_grid(){cjTable( '#grid','subcon_out_check_list?s=0&dates='+Math.floor(Math.random()*9999+1)+'&raw=<?php echo $raw;?>',////url of data source
							 '序号,出库日期,物料id,品名,全名,规格,规格2,状态,发出数量,单位,批号,生产单号,审核日期,成品,加工商名,返回的品名,入库数量,入库批准日期',////表格标题题
							 '400px','currency_id,warehouse,supplier_ids,gid,gmaterial_id,hmaterial_id');                                    ///表格高度,需要隐藏的td
                            }
	   function build_search_grid(search_string){cjTable( '#grid','subcon_out_check_list?s=0'+string+'&raw=<?php echo $raw;?>',////url of data source
							 '序号,出库日期,物料id,品名,全名,规格,规格2,状态,发出数量,单位,批号,生产单号,审核日期,成品,加工商名,返回的品名,入库数量,入库批准日期',////表格标题题
							 '400px','currency_id,warehouse,supplier_ids,gid,gmaterial_id,hmaterial_id');                                    ///表格高度,需要隐藏的td
                            }
	  ...
	  $("#revocation").click(
	   ...
	   $("#correct_material_name").keyup(function(){
       	  		  ...
       	  		  $("#stat").click(function(){///汇总统计
						   ...
						   $.getJSON('subcon_out_check?s=1&dates='+Math.floor(Math.random()*9999+1)+'&stat=1&'+string,
									 function(result)
									                { var sum="";
...
<div id=main>
  <h2><?php if($raw=='raw') {echo "原材料委外加工对账";}else{echo '半成品委外加工对账';}?></h2><p></p>
    <div id=main_left>
